...
from trugs_folder.utils import load_graph
...
class TestTfind:
    ...
    def test_find_by_type(self, tmp_path):
        _init_project(tmp_path)
        result = tfind(tmp_path, node_type="COMPONENT", format="json")
        assert isinstance(result, list)
        assert all(item["type"] == "COMPONENT" for item in result)
    ...
    def test_find_by_type_document(self, tmp_path):
        ...
        assert names == {"README.md", "config.yaml"}
    ...
    def test_find_text_format(self, tmp_path):
        _init_project(tmp_path)
        result = tfind(tmp_path, node_type="COMPONENT", format="text")
