...
from trugs_folder.tdim import tdim, tdim_add, tdim_remove, tdim_list, tdim_set
from trugs_folder.utils import load_graph, get_node_by_id
...
def _non_contains(result):
    """Edges excluding the scan-created 'contains' baseline (#53) —
    the mechanical edges tinit now correctly emits per child."""
    return [e for e in result["edges"] if e["relation"] != "contains"]
...
class TestTlink:
    ...
    def test_create_edge_with_properties(self, tmp_path):
        ...
        result = tlink(tmp_path, "main_py", "utils_py", "DEPENDS_ON",
                       properties=props)
        assert _non_contains(result)[0]["properties"] == props
    ...
    def test_multiple_edges_different_relations(self, tmp_path):
        ...
        assert len(_non_contains(result)) == 2
    ...
    def test_create_edge_with_weight_zero(self, tmp_path):
        ...
        assert _non_contains(result)[0]["weight"] == 0.0
    ...
    def test_create_edge_with_weight_one(self, tmp_path):
        ...
        assert _non_contains(result)[0]["weight"] == 1.0
    ...
    def test_create_edge_without_weight(self, tmp_path):
        ...
        assert "weight" not in _non_contains(result)[0]
...
class TestTunlink:
    ...
    def test_remove_edge(self, tmp_path):
        ...
        assert len(_non_contains(result)) == 0
