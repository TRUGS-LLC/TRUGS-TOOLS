...
class TestScanExamples:
    ...
    @pytest.mark.skipif(
        sys.platform == "darwin",
        reason=(
            "macOS HFS+/APFS default to case-insensitive filesystems. "
            "When this test creates 'examples/' under tmp_path but an "
            "'EXAMPLES/' already exists higher in the tree (or is created "
            "by sibling tests), the FS preserves the existing case and "
            "_scan_examples returns 'EXAMPLES/' instead of 'examples/'. "
            "See AAA Phase 8 finding F10 / followup AF-7."
        ),
    )
...
class TestIntegrationTrugsTools:
    ...
    def trugs_tools_path(self):
        ...
        trugs_tools = here.parent.parent  # repo root (tests moved into trugs-folder/tests, #2416 SP2)
...
class TestTestCountSource:
    ...
    def test_pytest_failure_fallback_file_count(self, tmp_path, caplog):
        ...
        with mock.patch("trugs_folder.folder_init.subprocess.run", return_value=fake_result):
            with caplog.at_level(logging.WARNING, logger="trugs_folder.folder_init"):
                node = _scan_tests(tmp_path, run_tests=True)
    ...
    def test_pytest_missing_fallback_file_count(self, tmp_path, caplog):
        ...
        with mock.patch(
            "trugs_folder.folder_init.subprocess.run",
