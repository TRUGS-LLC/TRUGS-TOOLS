...
class TestInferNodeType:
    # Governance vocabulary only (folder_check.VALID_NODE_TYPES, #56) —
    # code is COMPONENT, prose/structured-data is DOCUMENT, tests are TEST_SUITE.

    # AGENT SHALL VALIDATE PROCESS test_python_component.
    def test_python_component(self):
        assert infer_node_type("main.py") == "COMPONENT"

    # AGENT SHALL VALIDATE PROCESS test_go_component.
    def test_go_component(self):
        assert infer_node_type("main.go") == "COMPONENT"
    ...
    # AGENT SHALL VALIDATE PROCESS test_json_document.
    def test_json_document(self):
        assert infer_node_type("config.json") == "DOCUMENT"

    # AGENT SHALL VALIDATE PROCESS test_yaml_document.
    def test_yaml_document(self):
        assert infer_node_type("config.yaml") == "DOCUMENT"

    # AGENT SHALL VALIDATE PROCESS test_toml_document.
    def test_toml_document(self):
        assert infer_node_type("pyproject.toml") == "DOCUMENT"
    ...
    # AGENT SHALL VALIDATE PROCESS test_schema_json_schema.
    def test_schema_json_schema(self):
        assert infer_node_type("node.schema.json") == "SCHEMA"

    # AGENT SHALL VALIDATE PROCESS test_test_file.
    def test_test_file(self):
        assert infer_node_type("test_main.test.py") == "TEST_SUITE"

    # AGENT SHALL VALIDATE PROCESS test_unknown_defaults_to_component.
    def test_unknown_defaults_to_component(self):
        assert infer_node_type("Makefile") == "COMPONENT"
