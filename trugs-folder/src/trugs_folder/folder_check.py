...
<trl>
PROCESS folder_check SHALL SCAN FILE folder_trug_json THEN VALIDATE RECORD node AND RECORD edge AGAINST DATA governance_spec.
</trl>
"""
...
class CheckResult:
    ...
    <trl>
    RECORD CheckResult SHALL REPRESENT DATA validation_outcome AND SHALL CONTAIN DATA errors AND DATA warnings FOR FILE folder_trug_json.
    </trl>
    """
...
def check_folder_trug(
    ...
    <trl>
    FUNCTION check_folder_trug SHALL READ FILE folder_trug_json THEN VALIDATE RECORD node AND RECORD edge THEN RETURN RECORD CheckResult.
    </trl>
    """
    ...
    if folder_node is not None:
        ...
        for edge in edges:
            if edge.get("from_id") == folder_id and edge.get("relation") == "contains":
    ...
    for edge in edges:
        ...
        if ":" not in str(to_id) and to_id not in node_ids:
            result.errors.append(f"Edge to_id '{to_id}' does not reference any node")
            ...
            for item in sorted(folder_dir.iterdir()):
                ...
                if item.name not in node_names:
                    result.warnings.append(
                        f"On-disk item '{item.name}' is not represented by any node"
                    )
    ...
    if folder_node is not None:
        ...
        if len(contains_list) == 0:
            result.warnings.append(
                f"FOLDER node '{folder_node.get('id', '?')}' has empty contains array"
            )
...
def find_all_folder_trugs(root: Union[str, Path]) -> List[Path]:
    ...
    <trl>
    FUNCTION find_all_folder_trugs SHALL SCAN FILE root_directory THEN RETURN DATA sorted_path_list SUBJECT_TO ZZZ_dirs AND hidden_dirs ARE EXCLUDED.
    </trl>
    """
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        dirnames[:] = [
            d for d in dirnames if not d.startswith("ZZZ_") and not d.startswith(".")
        ]
...
def check_all(
    ...
    <trl>
    FUNCTION check_all SHALL RESOLVE DATA path_list THEN CHECK FILE folder_trug_json FOR EACH path THEN RETURN DATA result_list.
    </trl>
    """
        ...
        for p in paths:
            ...
            if p.is_dir():
                ...
                if candidate.exists():
                    ...
                    r = CheckResult(str(p))
                    r.errors.append(f"No folder.trug.json found in directory: {p}")
...
def format_text(results: List[CheckResult], quiet: bool = False) -> str:
    ...
    <trl>
    FUNCTION format_text SHALL RENDER DATA result_list TO DATA human_readable_text THEN RETURN DATA formatted_string.
    </trl>
    """
...
def format_json(results: List[CheckResult]) -> str:
    ...
    <trl>
    FUNCTION format_json SHALL MAP DATA result_list TO DATA json_object THEN RETURN DATA json_string.
    </trl>
    """
