...
<trl>
PROCESS tlink SHALL VALIDATE RECORD edge THEN WRITE RECORD edge TO DATA graph.
</trl>
"""
...
def tlink(
    ...
    <trl>
    FUNCTION tlink SHALL VALIDATE RECORD node AND RECORD relation THEN APPEND RECORD edge TO DATA graph.
    </trl>
    """
    ...
    for existing_edge in trug.get("edges", []):
        if (
            existing_edge.get("from_id") == from_id
            and existing_edge.get("to_id") == to_id
            and existing_edge.get("relation") == relation
        ):
            raise ValueError(
    ...
    if weight is not None:
        if (
            ...
            raise ValueError(
                f"Weight must be a number between 0.0 and 1.0, got {weight}"
            )
...
def tunlink(
    ...
    <trl>
    FUNCTION tunlink SHALL FILTER RECORD edge FROM DATA graph THEN WRITE DATA graph SUBJECT_TO RECORD edge EXISTS.
    </trl>
    """
    dirpath = Path(directory).resolve()
    trug = load_graph(dirpath)
