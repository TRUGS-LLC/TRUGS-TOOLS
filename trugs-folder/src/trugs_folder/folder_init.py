...
<trl>
PROCESS folder_init SHALL SCAN FILE system THEN BUILD RECORD trug_graph AND RETURN DATA folder_trug_json.
</trl>
"""
...
def _scan_documents(folder_path: Path) -> List[dict]:
    ...
    spec_pattern = re.compile(
        r"_SPEC\.md$|_SPECIFICATION\.md$|^SPEC_|^SPECIFICATION_", re.IGNORECASE
    )
    nodes: List[dict] = []
...
def _scan_components(folder_path: Path) -> List[dict]:
    ...
    for entry in entries:
        ...
        py_files = [
            ...
            if not any(
                part in _COMPONENT_SKIP_PARTS
                for part in f.relative_to(entry).parts[:-1]
            )
        ]
        if not py_files:
            continue
...
def init_folder_trug(
    ...
    <trl>
    FUNCTION init_folder_trug SHALL SCAN FILE system THEN BUILD RECORD trug_graph AND RETURN DATA folder_trug_dict SUBJECT_TO path IS DIRECTORY.
    </trl>
    """
...
def find_folders_without_trug(root: Union[str, Path]) -> List[Path]:
    ...
    <trl>
    FUNCTION find_folders_without_trug SHALL SCAN FILE system THEN FILTER ALL RECORD directory WHERE folder_trug_json IS ABSENT AND RETURN DATA path_list.
    </trl>
    """
