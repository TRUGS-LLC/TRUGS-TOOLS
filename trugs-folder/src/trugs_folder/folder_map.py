...
<trl>
PROCESS folder_map SHALL SCAN FILE folder_trug_json UNDER DATA root_directory THEN BUILD RECORD root_graph FROM RECORD cross_folder_edge.
</trl>
"""
...
from trugs_folder.folder_check import find_all_folder_trugs
...
class FolderInfo:
    ...
    <trl>
    RECORD FolderInfo SHALL REPRESENT DATA folder_trug AND SHALL EXPOSE RECORD folder_node AND DATA node_ids.
    </trl>
    """
...
class MapResult:
    ...
    <trl>
    RECORD MapResult SHALL REPRESENT RESULT map_folder_trugs AND SHALL EXPOSE DATA resolved_edges AND DATA unresolved_edges AND DATA root_graph.
    </trl>
    """
...
def _find_orphaned_folders(
    ...
    orphaned = sorted(alias for alias in folders if alias not in connected)
    return orphaned
...
def map_folder_trugs(
    ...
    <trl>
    FUNCTION map_folder_trugs SHALL SCAN FILE folder_trug_json THEN RESOLVE RECORD cross_folder_edge THEN BUILD RECORD root_graph AND RETURN RECORD MapResult.
    </trl>
    """
    ...
    if not trug_paths:
        raise FileNotFoundError(f"No folder.trug.json files found under {root_path}")
    ...
    result.changes.append(f"Mapped {result.folder_count} folder TRUGs → root graph")
    result.changes.append(f"Resolved: {len(resolved)} cross-folder edges")
    if unresolved:
        result.changes.append(f"Unresolved: {len(unresolved)} edges")
        for e in unresolved:
    ...
    result.changes.append(f"Root graph: {node_count} nodes, {edge_count} edges")
