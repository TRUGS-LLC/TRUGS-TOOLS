...
<trl>
PROCESS tadd SHALL APPEND RECORD node TO FILE folder_trug_json SUBJECT_TO RECORD node NOT EXISTING IN DATA graph.
</trl>
"""
...
def tadd(
    ...
    <trl>
    FUNCTION tadd SHALL LOAD FILE folder_trug_json THEN APPEND RECORD node FOR EACH DATA filename AND WRITE FILE folder_trug_json THEN RETURN DATA graph.
    </trl>
    """
    ...
    for filename in files:
        ...
        node: Dict[str, Any] = {
            ...
            "properties": {
                "name": filename,
                "purpose": purpose
                or f"{'Directory' if filepath.is_dir() else 'File'}: {filename}",
            },
