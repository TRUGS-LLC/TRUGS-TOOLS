...
<trl>
PROCESS folder_sync SHALL SCAN FILE folder_trug_json THEN PATCH RECORD node WITH DATA factual_properties AND SHALL_NOT MODIFY DATA human_curated_properties.
</trl>
"""
...
class SyncResult:
    ...
    <trl>
    RECORD SyncResult SHALL REPRESENT DATA sync_outcome AND SHALL EXPOSE RECORD updated_nodes AND RECORD new_nodes AND RECORD stale_nodes AND RECORD pruned_nodes.
    </trl>
    """
...
def sync_folder_trug(
    ...
    <trl>
    FUNCTION sync_folder_trug SHALL LOAD FILE folder_trug_json THEN SCAN FILE filesystem THEN PATCH RECORD node WITH DATA factual_properties THEN RETURN RECORD SyncResult.
    </trl>
    """
    ...
    for node in existing_nodes:
        ...
        if node_id in fresh_by_id:
            ...
            if props.get("stale"):
                ...
                result.changes.append(f"Cleared stale: {node_id} (file returned)")
            ...
            if not props.get("stale"):
                ...
                result.changes.append(f"Stale: {node_id} (file not found)")
            else:
    ...
    if prune_after > 0:
        ...
        for node in list(existing_nodes):
            props = node.get("properties", {})
            if props.get("stale") and props.get("stale_count", 0) >= prune_after:
                pruned_ids.append(node["id"])
        # Remove pruned nodes (preserve order, deterministic)
        if pruned_ids:
            pruned_set = set(pruned_ids)
            existing_nodes[:] = [n for n in existing_nodes if n["id"] not in pruned_set]
            ...
            if folder_node is not None:
                contains = folder_node.get("contains", [])
                folder_node["contains"] = [c for c in contains if c not in pruned_set]
    ...
    for node in existing_nodes:
        ...
        if status == "NOT_STARTED":
            ...
            loc = props.get("loc", 0)
            file_exists = node["id"] in fresh_by_id or _node_file_exists(
                folder_path, props, node_type
            )
    ...
    if folder_node is not None:
        ...
        folder_changes = _update_folder_metadata(folder_node, aaa_meta, pyproject_meta)
        if folder_changes:
...
def _update_folder_metadata(
    ...
    if new_version:
        ...
        if old_version != new_version:
            ...
            changes.append(f"Updated: {node_id} (version: {old_version}→{new_version})")
...
def _update_top_level_metadata(
    ...
    if "description" in pyproject_meta:
        ...
        if old != new:
            ...
            changes.append("Updated: top-level description")
