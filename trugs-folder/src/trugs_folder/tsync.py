...
<trl>
PROCESS tsync SHALL SCAN FILE directory THEN SYNC RECORD trug_graph TO DATA filesystem_state.
</trl>
"""
...
def tsync(
    ...
    <trl>
    FUNCTION tsync SHALL LOAD RECORD trug_graph FROM FILE directory THEN RETURN DATA sync_result SUBJECT_TO dry_run EQUALS false FOR WRITE.
    </trl>
    """
    ...
    for existing_node in trug.get("nodes", []):
        name = existing_node.get("properties", {}).get("name", "")
        if name:
