...
<trl>
PROCESS tdim SHALL MANAGE RECORD dimension ON RECORD trug_graph AND SHALL DISPATCH DATA action TO FUNCTION tdim_add OR FUNCTION tdim_remove OR FUNCTION tdim_list OR FUNCTION tdim_set.
</trl>
"""
...
def tdim_add(
    ...
    <trl>
    FUNCTION tdim_add SHALL LOAD RECORD trug_graph FROM FILE directory THEN WRITE RECORD dimension TO RECORD trug_graph AND RETURN RECORD trug_graph.
    </trl>
    """
...
def tdim_remove(
    ...
    <trl>
    FUNCTION tdim_remove SHALL LOAD RECORD trug_graph FROM FILE directory THEN REMOVE RECORD dimension AND RETURN RECORD trug_graph SUBJECT_TO dimension NOT USED BY ANY RECORD node OR force EQUALS TRUE.
    </trl>
    """
    dirpath = Path(directory).resolve()
    trug = load_graph(dirpath)
    ...
    using = [n["id"] for n in trug.get("nodes", []) if n.get("dimension") == name]
...
def tdim_list(
    ...
    <trl>
    FUNCTION tdim_list SHALL LOAD RECORD trug_graph FROM FILE directory THEN RETURN DATA dimensions WITH RECORD node COUNT PER RECORD dimension.
    </trl>
    """
    dirpath = Path(directory).resolve()
    trug = load_graph(dirpath)
...
def tdim_set(
    ...
    <trl>
    FUNCTION tdim_set SHALL RESOLVE RECORD node BY node_id AND WRITE RECORD dimension TO RECORD node THEN RETURN RECORD trug_graph.
    </trl>
    """
    dirpath = Path(directory).resolve()
    trug = load_graph(dirpath)
...
def tdim(
    ...
    **kwargs,
    ...
    <trl>
    FUNCTION tdim SHALL MAP DATA action TO FUNCTION tdim_add OR FUNCTION tdim_remove OR FUNCTION tdim_list OR FUNCTION tdim_set THEN RETURN RESULT.
    </trl>
    """
